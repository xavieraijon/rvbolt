...
class Maid {
			...
			if ($this->fullDocument) {
				...
				for ($i = 0; $i < $dstParent->childNodes->length; ++$i) {
					switch ($this->outputFormat) {
						...
						case 'html':
						default:
							$html .= self::renderFragment($dstParent->childNodes->item($i));
							break;
					}
				}
				return $html;
	 ...
	 * because PHP versions before 5.3.6 don't support writing out individual
	 ...
	 */
	public static function renderFragment(\DOMNode $node) {
